...
public class QRCodeApp {
    ...
    // حفظ رمز الاستجابة السريعة في قاعدة البيانات
    // حفظ رمز الاستجابة السريعة في قاعدة البيانات بدون JOptionPane
    public static void saveQRCodeToDatabase(String data, byte[] qrImage) {
        ...
        try (Connection conn = connect(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            pstmt.executeUpdate();
            // طباعة رسالة النجاح في Console
            System.out.println("\033[0;34m[1]QR Code saved successfully in the database!\033[0m");
        } catch (SQLException e) {
            System.err.println("Error saving QR Code to the database: " + e.getMessage());
        }
    }
    ...
    public static void deleteQRCodeFromDatabase(String data) {
        ...
        try (Connection conn = connect(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            if (rowsAffected > 0) {
                System.out.println("\033[0;34m[1]QR Code deleted successfully from the database!\033[0m");
            } else {
                System.out.println("\033[0;31m[1]QR Code not found in the database.\033[0m");
            }
        } catch (SQLException e) {
    ...
    // حذف رمز الاستجابة السريعة من قاعدة البيانات باستخدام المعرف (ID)
    public static void deleteQRCode(int id) {
        ...
        try (Connection conn = connect(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            int rowsAffected = pstmt.executeUpdate();
            if (rowsAffected > 0) {
                System.out.println("QR Code with ID " + id + " deleted successfully from the database!");
            } else {
                System.out.println("QR Code with ID " + id + " not found in the database.");
            }
        } catch (SQLException e) {
            System.err.println("Error deleting QR Code: " + e.getMessage());
        }
